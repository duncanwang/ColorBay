...
2、在锁仓阶段的10:09:59时，张三试图去释放自己的激励，切换到张三账号下：
```
release() //此时释放数量应该为0
```
...
```
4、在释放阶段的10:20:00时（或之后），张三试图去释放自己的激励，切换到张三账号`0x147...c160c`下，重复第3步操作，张三应该拿到5000个CLB

5、在释放阶段的10:30:00时（即释放结束时间）（或之后），张三试图去释放自己的激励，切换到张三账号`0x147...c160c`下，重复第3步操作，张三应该拿到了10000个CLB
