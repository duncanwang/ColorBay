...
contract Colorbay is PausableToken, MintableToken, BurnableToken {
    ...
    function test() public view returns (uint256, uint256) {
        uint256 closingTime = now + 30 days;
...
contract Crowdsale {
  ...
  function buyTokens(address _beneficiary) public payable {
    ...
    _preValidatePurchase(_beneficiary, weiAmount); //参数校验

    // calculate token amount to be created
    uint256 tokens = _getTokenAmount(weiAmount); //计算将要转账的token数量

    // update state
    weiRaised = weiRaised.add(weiAmount); //记录募集总数

    _processPurchase(_beneficiary, tokens); //转币
    emit TokenPurchase(
      msg.sender,
      _beneficiary,
      weiAmount,
...
contract TimedCrowdsale is Crowdsale {
  ...
  function _preValidatePurchase(
    ...
    onlyWhileOpen
  {
    super._preValidatePurchase(_beneficiary, _weiAmount); //参数校验
  }
